...
/*
 * Durable Subgraph Matching (TD-tree + path enumeration)
 * with:
 *   - Root selection by S_temp (Eq. (2)(3)): S_temp(u) = deg_Q(u) / ( |V_L(u)| * Adur(u) )
 *   - Temporal graph filter during snapshot merge: keep an edge (u,v) only if it has
 *     at least one pair of consecutive snapshots (length >= 2 run).
 *
 * Notes:
 * - Adur(u) is computed on the filtered graph as the average #active snapshots of vertices
 *   with label L = label(u). dur(v) is |{t | v appears at t in filtered graph}|.
 * - Stage-1 growth, Stage-2 trimming, path enumeration & join follow TD-tree paradigm.
 */
...
int main(int argc, char** argv) {
    ...
    // ===== CLI (논문 범위 내에서 필요한 인자만 유지) =====
    string input_path, query_path, output_path = "tdtree_output.txt";
    unsigned int seed = 42;
    int k = 3;                 // 최소 지속 스냅샷 수 (matching 단계에서 사용)
    bool directed = false;     // 기본 무향
    ...
    for (int i=1;i<argc;++i){
        ...
        if ((arg=="-i"||arg=="--input") && i+1<argc) input_path=argv[++i];
            ...
            cerr<<"Usage: "<<argv[0]<<" -i edges.txt -q query.txt [-o out.txt]\n"
                <<"  [-k 3] [--directed] [--no-strict-root-filter]\n";
            return 0;
        }
    }
    ...
    for (auto &e: edges){ vset.insert(e.u); vset.insert(e.v); }
    vector<int> vertices(vset.begin(), vset.end());
    sort(vertices.begin(), vertices.end());

    // 데이터 정점 라벨 무작위(A~E) — 입력 전처리
    ...
    // ---- (1) 스냅샷 병합 + 시간 필터(연속 길이 2 미만 간선 제거) ----
    unordered_map<int, unordered_map<int, vector<int>>> adj_raw;
    for (auto &e: edges){
        adj_raw[e.u][e.v].push_back(e.t);
        if (!directed) adj_raw[e.v][e.u].push_back(e.t);
    }
    for (auto &kv1 : adj_raw){
    ...
    const int FILTER_MIN_CONSEC = 2; // at least two consecutive snapshots
    unordered_map<int, unordered_map<int, vector<int>>> adj; // filtered
    for (auto &kv1 : adj_raw){
        int u = kv1.first;
        for (auto &kv2 : kv1.second){
            int v = kv2.first;
            const auto &tv = kv2.second;
            bool has_consecutive = false;
            for (size_t i=1;i<tv.size();++i){
                if (tv[i] == tv[i-1] + 1) { has_consecutive = true; break; }
            }
            if (has_consecutive){
                adj[u][v] = tv; // keep full list (not only consecutive times)
            }
        }
    }

    // all times from FILTERED graph
    vector<int> allTimesUniverse;
    for (auto &kv1 : adj){
        for (auto &kv2 : kv1.second){
            for (int t : kv2.second) allTimesUniverse.push_back(t);
        }
    }
    sort(allTimesUniverse.begin(), allTimesUniverse.end());
    allTimesUniverse.erase(unique(allTimesUniverse.begin(), allTimesUniverse.end()), allTimesUniverse.end());

    // 정점 지속시간 dur(v): filtered 간선 기반
    ...
    for (auto &kv1 : adj){
        ...
        for (auto &kv2 : kv1.second){
            int v = kv2.first;
            const auto &tv = kv2.second;
            for (int t : tv){ vertexTimes[u].push_back(t); vertexTimes[v].push_back(t); }
        }
    }
    ...
    // degree / neighbor label set (filtered)
    ...
    // ===== 루트 선택 by S_temp: deg_Q / ( |V_L| * Adur ) =====
    auto avgDurForLabel = [&](char L)->double{
    ...
    char rootLabel = 0;
    double bestSel = numeric_limits<double>::infinity();
    ...
    for (auto &kv : qDeg){
        ...
        double Adur = avgDurForLabel(L);
        double denom = (double)cand * Adur;
        if (denom <= 0.0) continue;
        double s = (double)d / denom;
        if (s<bestSel || (fabs(s-bestSel)<1e-12 && (cand<bestCand || (cand==bestCand && L<rootLabel)))){
            bestSel=s; bestCand=cand; rootLabel=L; bestAdur=Adur;
        }
    }
    if (rootLabel==0){ cerr<<"cannot choose root (no valid label with nonzero Adur and candidates)\n"; return 1; }
    ...
    auto selVal=[&](char L)->double{
        ...
        double Ad = avgDurForLabel(L);
        if (d==0||c==0||Ad<=0) return 1e100;
        return double(d) / ( (double)c * Ad );
    };
    ...
    for (auto &e : nonTreeEdges){
        ...
        else { nonTreeAnc[b].push_back(a); nonTreeAnc[a].push_back(b); } // 안전장치
    }
    ...
    auto existsEdgeToNodeCandidates = [&](int v0, char ancLabel)->bool{
        ...
        for (auto &kv : it->second){
            int w = kv.first;
            if (!A.bloom.possiblyContains((uint64_t)w)) continue; // fast reject
            if (!A.memberSet.count(w)) continue;                  // 정확 검증
            return true;
        }
        return false;
    };
    ...
    function<void(char,int,const vector<int>&)> fill_node =
        [&](char c, int vpar, const vector<int>& TS_in){
            ...
            if (itNei != adj.end()) {
                for (auto &kv : itNei->second) {
                    int w = kv.first; const auto &edgeTS = kv.second;
                    if (dataLabel[w] != c) continue;
                    ...
                    Cell cell; cell.v = w;
                    if (node.isLeaf) cell.ts = TSnew; // leaf stores TS
            ...
            if (!blk.vcand.empty()) {
                ...
                if (!node.isLeaf) {
                    ...
                    for (const auto &cell : last.vcand) {
                        vector<int> TSedge = adj[vpar][cell.v];
    ...
    for (char c : TD[rootLabel].children)
        for (int vpar : TD[rootLabel].rootVcand)
            fill_node(c, vpar, allTimesUniverse);
    ...
    removeBlocksByParent = [&](char nodeLabel, int vpar){
        ...
        node.rebuildMemberSet();
        node.rebuildBloom(m_bits_nodes, 3, ((uint64_t)seed<<1) ^ nodeLabel, ((uint64_t)seed<<2) ^ (nodeLabel*131ULL));
        node.rebuildIndex();
    };
    ...
    removeCandidateCascade = [&](char uLabel, int v){
        ...
        if (u.isRoot){
            ...
            u.rebuildMemberSet();
            u.rebuildBloom(m_bits_nodes, 3, ((uint64_t)seed<<1) ^ uLabel, ((uint64_t)seed<<2) ^ (uLabel*131ULL));
    ...
    // 경로 cardinality(leaf의 Σ_b |b.Vcand|)로 오름차순 정렬
    vector<pair<int,int>> order; // (card, idx)
    ...
    auto enumerate_path = [&](const vector<char>& path)->vector<Match>{
        ...
        vector<int> TS0 = allTimesUniverse; // universe from filtered graph
        asg[rootLabel] = -1;
    ...
    // ===== 경로 간 조인 (공유 라벨로 키 조인 + 모든 질의 간선 시간제약) =====
    ...
    auto satisfies_all_query_edges = [&](const unordered_map<char,int>& asg, vector<int>& TS)->bool{
        for (auto &e : qEdges){
            char a=e.first, b=e.second;
    ...
    for (size_t oi=1; oi<order.size(); ++oi){
        ...
        vector<char> keyLabels = set_intersection_labels(pSet, cSet); // 공유 라벨
        ...
        for (const auto &pm : pathMatches[idx]){
            ...
            for (int ci : it->second){
                ...
                if (ok){
                    ...
                    ok = satisfies_all_query_edges(merged, TS);
                    if (!ok) continue;
    ...
    fout << "=== Durable Subgraph Matching (TD-tree + path enumeration, S_temp root + time filter) ===\n";
    ...
    fout << "Root label: " << rootLabel << "  S_temp = deg_Q("<<rootLabel<<")/( |V("<<rootLabel<<")| * Adur("<<rootLabel<<") ) = "
         << fixed << setprecision(6) << bestSel << "\n";
    fout << "  |V("<<rootLabel<<")|="<<bestCand<<",  Adur("<<rootLabel<<")="
         << fixed << setprecision(3) << bestAdur << "\n";
    ...
    fout << "\n[Timing (ms)]\n";
    fout << "Preprocess (read + build + time-filter): " << fixed << setprecision(3) << t_pre  << "\n";
    fout << "Root selection (S_temp):                 " << t_root << "\n";
