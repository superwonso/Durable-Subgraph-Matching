...
int main(int argc, char** argv) {
    ...
    // 4-1) 정점별 등장 스냅샷 집합 vertexTimes: dur(v) = |{t : v가 t에 등장}|
    unordered_map<int, vector<int>> vertexTimes;
    {
        for (const auto& e : edges) {
            vertexTimes[e.u].push_back(e.t);
            vertexTimes[e.v].push_back(e.t);
        }
    ...
    // 8) Selectivity 기반 루트 라벨 선택 (변경된 공식):
    //     S_temp(u) = deg_Q(u) / ( |V_L(u)| * Adur(u) )
    //     Adur(u) = avg_v in V_L(u) dur(v),  dur(v)=vertexTimes[v].size()
    char rootLabel = 0;
    double bestSelect = numeric_limits<double>::infinity();
    int bestCandCount = INT_MAX;
    double bestAdur = 0.0;

    auto avgDurForLabel = [&](char lbl) -> double {
        auto it = dataVerticesByLabel.find(lbl);
        if (it == dataVerticesByLabel.end() || it->second.empty()) return 0.0;
        long long sum = 0;
        for (int v : it->second) {
    ...
    for (const auto& kv : qDegree) {
        ...
        if (degQ == 0 || candCount == 0) continue;

        double Adur = avgDurForLabel(lbl); // 평균 수명
        double denom = (double)candCount * Adur;
        if (denom <= 0.0) continue;        // 평균 수명이 0이면 후보로 부적합

        double sel = (double)degQ / denom; // 새 selectivity
        if (sel < bestSelect ||
            (fabs(sel - bestSelect) < 1e-12 &&
             (candCount < bestCandCount || (candCount == bestCandCount && lbl < rootLabel)))) {
            bestSelect = sel;
    ...
    if (rootLabel == 0) {
        cerr << "Error: could not select a root (no valid label with nonzero Adur and candidates).\n";
        return 1;
    }
    ...
    // 10) 후보 집합 구성 (strict 필터용 데이터량도 계산)
    ...
    if (strict_root_filter) {
        // deg(v) >= deg_Q(root), 이웃라벨수 >= |N_Q(root)|, 정점 지속성 >= k
    ...
    // 새 selectivity 공식으로 출력
    fout << fixed << setprecision(6);
    fout << "Selected root query-vertex label: " << rootLabel
         << "  (S_temp = deg_Q(" << rootLabel << ") / (|V(" << rootLabel
         << ")| * Adur(" << rootLabel << ")) = " << bestSelect << ")\n";
    fout << "  |V(" << rootLabel << ")| = " << bestCandCount
         << ", Adur(" << rootLabel << ") = " << bestAdur
         << ", deg_Q(" << rootLabel << ") = " << qDegree[rootLabel] << "\n";
    ...
    // 루트 후보 및 자식 요약
    fout << "Root node [" << rootLabel << "] candidates after filtering (|Vcand(root)|): "
    ...
    for (char cl : children) {
        auto& blocks = blocksPerChild[cl];
        size_t totalEdges = 0;
        for (auto& p : blocks) totalEdges += p.second.size();
        ...
        fout << "Total candidate edges (Σ_b |b.Vcand|) = " << totalEdges << "\n";
