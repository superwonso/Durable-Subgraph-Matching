...
QueryDecomposition decomposeQuery(
    ...
    const std::array<double, kLabelCount>& label_average_lifespans) {
    ...
    result.temporal_selectivity.assign(
    ...
    for (int query_vertex = 0; query_vertex < query_vertex_count; ++query_vertex) {
        ...
        const double average_lifespan = label_average_lifespans[label];
        if (candidate_count == 0 || average_lifespan <= 0.0) continue;

        // 260408_cdh.pdf, Eq. (3.2):
        // S_temp(u) = deg_Q(u) / (|V_L(u)| * A_dur(u)), root = argmin.
        const double score = static_cast<double>(degree) /
            (static_cast<double>(candidate_count) * average_lifespan);
        result.temporal_selectivity[static_cast<std::size_t>(query_vertex)] = score;

        const bool smaller = result.root < 0 ||
            score < result.temporal_selectivity[static_cast<std::size_t>(result.root)];
        const bool tied = result.root >= 0 &&
            std::abs(score - result.temporal_selectivity[static_cast<std::size_t>(result.root)]) <= 1e-15;
    ...
    std::function<void(int)> dfs = [&](int query_vertex) {
        ...
        std::sort(neighbors.begin(), neighbors.end(), [&](int lhs, int rhs) {
            const double lhs_score = result.temporal_selectivity[static_cast<std::size_t>(lhs)];
            const double rhs_score = result.temporal_selectivity[static_cast<std::size_t>(rhs)];
